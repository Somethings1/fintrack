...
docker run -d --name fintrack-api --network fintrack-ci --network-alias api   --read-only --cap-drop ALL --security-opt no-new-privileges --tmpfs /tmp:rw,noexec,nosuid,size=16m   -e APP_ENV=production -e LEDGER_CURRENCY=USD -e 'MONGO_URI=mongodb://mongo:27017/?replicaSet=rs0'   -e MONGO_DATABASE=fintrack_smoke -e SUPABASE_URL=https://test.supabase.co   -e SUPABASE_ANON_KEY=public-test-only -e ALLOWED_ORIGINS=https://app.test   fintrack-api:ci
