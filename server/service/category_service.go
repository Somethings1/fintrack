...
func AddCategory(ctx context.Context, c model.Category) (interface{}, error) {
	c.Owner = util.UserID(ctx)
	if c.Owner == "" {
		return nil, errors.New("missing authenticated owner")
	}
	currency, err := money.Resolve(ctx, c.Currency)
	if err != nil {
		return nil, err
	}
	c.Currency = currency
	...
	if err := money.Validate(c.Budget, currency); err != nil || c.Budget < 0 {
		if err != nil {
			return nil, err
		}
		return nil, errors.New("negative budget")
	}
	c.ID = primitive.NewObjectID()
	c.LastUpdate = time.Now().UTC()
	_, err = util.DB.ExecContext(ctx, `INSERT INTO categories(id,owner,currency,type,icon,name,budget_micros,last_update,is_deleted) VALUES($1,$2,$3,$4,$5,$6,$7,$8,false)`, c.ID.Hex(), c.Owner, c.Currency, c.Type, c.Icon, c.Name, int64(c.Budget), c.LastUpdate)
	if err != nil {
		return nil, err
	}
	socket.BroadcastFromContext(ctx, map[string]interface{}{"collection": "categories", "action": "create", "detail": c})
	return c.ID, nil
}

func UpdateCategory(ctx context.Context, id primitive.ObjectID, c model.Category) error {
	if _, err := money.Resolve(ctx, c.Currency); err != nil {
		return err
	}
	...
	socket.BroadcastFromContext(ctx, map[string]interface{}{"collection": "categories", "action": "update"})
	return nil
}
func DeleteCategory(ctx context.Context, id primitive.ObjectID) error {
	return archiveCategory(ctx, id)
}
...
func SyncCategories(ctx context.Context, user string, since, after time.Time, afterID primitive.ObjectID, limit int) ([]model.Category, bool, error) {
	...
	rows, err := util.DB.QueryContext(ctx, `SELECT `+categoryCols+` FROM categories WHERE owner=$1 AND last_update >= $2 AND ($3::timestamptz IS NULL OR (last_update,id)>($3,$4)) ORDER BY last_update,id LIMIT $5`, user, since, nullTime(after), nullString(id), limit+1)
	if err != nil {
		return nil, false, err
	}
	defer rows.Close()
	out := make([]model.Category, 0, limit+1)
	for rows.Next() {
		v, e := scanCategory(rows)
		if e != nil {
			return nil, false, e
		}
		out = append(out, v)
