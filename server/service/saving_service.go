...
func GetSavingByID(ctx context.Context, id string) (model.Saving, error) {
	...
	return scanSaving(util.DB.QueryRowContext(ctx, `SELECT `+savingCols+` FROM financial_accounts WHERE id=$1 AND owner=$2 AND kind='saving' AND is_deleted=false`, id, util.UserID(ctx)))
}
...
func UpdateSaving(ctx context.Context, id primitive.ObjectID, v model.Saving) error {
	...
	socket.BroadcastFromContext(ctx, map[string]interface{}{"collection": "savings", "action": "update"})
	return nil
}
func DeleteSaving(ctx context.Context, id primitive.ObjectID) error {
	return archiveFinancialAccount(ctx, "saving", "savings", id)
}
func SyncSavings(ctx context.Context, user string, since, after time.Time, afterID primitive.ObjectID, limit int) ([]model.Saving, bool, error) {
	after, id := afterArgs(after, afterID)
	rows, err := util.DB.QueryContext(ctx, `SELECT `+savingCols+` FROM financial_accounts WHERE owner=$1 AND kind='saving' AND last_update >= $2 AND ($3::timestamptz IS NULL OR (last_update,id)>($3,$4)) ORDER BY last_update,id LIMIT $5`, user, since, nullTime(after), nullString(id), limit+1)
	if err != nil {
		return nil, false, err
	}
	defer rows.Close()
	out := make([]model.Saving, 0, limit+1)
	for rows.Next() {
		v, e := scanSaving(rows)
		if e != nil {
			return nil, false, e
