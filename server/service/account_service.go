...
func GetAccountByID(ctx context.Context, id string) (model.Account, error) {
	if _, err := primitive.ObjectIDFromHex(id); err != nil { return model.Account{}, err }
...
func AddAccount(ctx context.Context, a model.Account) (interface{}, error) {
	a.Owner = util.UserID(ctx); if a.Owner == "" { return nil, errors.New("missing authenticated owner") }
	currency, err := money.Resolve(ctx, a.Currency); if err != nil { return nil, err }; a.Currency = currency
	if err := money.Validate(a.Balance, currency); err != nil { return nil, err }
	a.ID = primitive.NewObjectID(); a.OpeningBalance = a.Balance; a.LastUpdate = time.Now().UTC(); a.IsDeleted = false
...
func UpdateAccount(ctx context.Context, id primitive.ObjectID, a model.Account) error {
	if _, err := money.Resolve(ctx, a.Currency); err != nil { return err }
	if err := money.Validate(a.Balance, money.Currency(ctx)); err != nil { return err }
	...
	if err != nil { return err }; n,_ := res.RowsAffected(); if n != 1 { return sql.ErrNoRows }
	socket.BroadcastFromContext(ctx, map[string]interface{}{"collection":"accounts","action":"update"}); return nil
}

func DeleteAccount(ctx context.Context, id primitive.ObjectID) error { return archiveFinancialAccount(ctx, "account", "accounts", id) }
...
func SyncAccounts(ctx context.Context, user string, since, after time.Time, afterID primitive.ObjectID, limit int) ([]model.Account, bool, error) {
	...
	if err != nil { return nil,false,err }; defer rows.Close()
	out := make([]model.Account,0,limit+1); for rows.Next(){ v,e:=scanAccount(rows); if e!=nil{return nil,false,e}; out=append(out,v) }
	if err:=rows.Err();err!=nil{return nil,false,err}; more:=len(out)>limit; if more{out=out[:limit]}; return out,more,nil
}
