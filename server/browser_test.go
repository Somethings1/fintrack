...
func TestBrowserHarness(t *testing.T) {
	...
	for email, id := range ids {
		name := "Alice"
		if strings.HasPrefix(email, "bob") {
			name = "Bob"
		}
		owned := money.WithCurrency(context.WithValue(ctx, util.UserIdKey, id), "USD")
		if _, err := service.AddAccount(owned, model.Account{Name: name + " Wallet", Icon: "W", Balance: money.Must("100")}); err != nil {
			t.Fatal(err)
		}
		if _, err := service.AddCategory(owned, model.Category{Name: "Food", Icon: "F", Type: "expense", Budget: money.Must("100")}); err != nil {
			t.Fatal(err)
		}
	}
	var mu sync.Mutex
	...
	mock := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/auth/v1/token" {
			...
			if json.NewDecoder(http.MaxBytesReader(w, r.Body, 4096)).Decode(&input) != nil || ids[input.Email] == "" || input.Password != "ci-only-password" {
				w.WriteHeader(400)
				_ = json.NewEncoder(w).Encode(map[string]string{"msg": "Invalid credentials"})
				return
			}
			id := ids[input.Email]
		...
		if user == nil {
			w.WriteHeader(401)
			_ = json.NewEncoder(w).Encode(map[string]string{"msg": "Invalid session"})
			return
		}
		...
		if r.URL.Path == "/rest/v1/profiles" {
			id := user["id"].(string)
			if r.URL.Query().Get("id") != "eq."+id {
				w.WriteHeader(403)
				return
			}
			if profiles[id] == nil {
				profiles[id] = map[string]interface{}{"id": id, "email": user["email"], "full_name": "CI User", "display_currency": "EUR", "display_floating_points": 3, "display_locale": "en-US", "avatar_path": "", "currency_position": "before"}
			}
			if r.Method == "PATCH" {
				var fields map[string]interface{}
				if json.NewDecoder(http.MaxBytesReader(w, r.Body, 4096)).Decode(&fields) != nil {
					w.WriteHeader(400)
					return
				}
				for _, key := range []string{"full_name", "avatar_path", "notification_income", "notification_expense", "display_locale", "display_currency", "display_floating_points", "currency_position", "updated_at"} {
			...
			if strings.Contains(r.Header.Get("Accept"), "vnd.pgrst.object") {
				...
				_ = json.NewEncoder(w).Encode([]interface{}{profiles[id]})
			}
			return
		}
		http.NotFound(w, r)
	...
	authListener, err := net.Listen("tcp", "127.0.0.1:8089")
	if err != nil {
		t.Fatal(err)
	}
	defer auth.Close()
	go func() { _ = auth.Serve(authListener) }()
	...
	apiListener, err := net.Listen("tcp", "127.0.0.1:8080")
	if err != nil {
		t.Fatal(err)
	}
	defer api.Close()
	go func() { _ = api.Serve(apiListener) }()
	<-ctx.Done()
}
